...
def main ():
    ...
    e = st.sidebar.number_input('Margem de erro (%)', key= float, min_value=1.0, max_value=100, step=0.5, format='%f')
    p = st.sidebar.number_input('Estimativa de verdadeira proporção (%)', key= int, min_value=10, value=50, max_value=100, step=10, format='%i')
