...
def main ():
    ...
    e = float((st.sidebar.number_input('Margem de erro (%)', key= float, min_value=1.0, max_value=100, step=0.5, format='%f')/100
    p = float((st.sidebar.number_input('Estimativa de verdadeira proporção (%)', key= int, min_value=10, value=50, max_value=100, step=10, format='%i')/100
